...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium transition-all duration-200 cursor-pointer disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
  {
    variants: {
      ...
      size: {
        default: "h-10 px-5 text-sm rounded-[6px]",
        sm: "h-8 px-4 text-xs rounded-[4px]",
        md: "h-10 px-5 text-sm rounded-[6px]",
        lg: "h-12 px-6 text-base rounded-[6px]",
        xl: "h-14 px-8 text-lg rounded-[6px]",
        icon: "h-10 w-10 rounded-[6px]",
        "icon-xs": "h-8 w-8 rounded-[4px] [&_svg]:size-3",
        "icon-sm": "h-9 w-9 rounded-[6px]",
        "icon-lg": "h-14 w-14 rounded-[6px] [&_svg]:size-6",
      },
    },
    defaultVariants: {
      variant: "default",
